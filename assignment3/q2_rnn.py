...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) < max_length:
            pad_data = sentence + (max_length - len(sentence)) * [zero_vector]
            pad_label = labels + (max_length - len(sentence)) * [zero_label]
            mask = len(sentence) * [True] + (max_length - len(sentence)) * [False]
        else:
            pad_data = sentence[:max_length]
            pad_label = labels[:max_length]
            mask = max_length * [True]
        ret.append((pad_data, pad_label, mask))
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOU CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(shape=(None, Config.max_length,Config.n_features), dtype=tf.int32, name="input")
        self.labels_placeholder = tf.placeholder(shape=(None, Config.max_length), dtype=tf.int32, name="label")
        self.mask_placeholder = tf.placeholder(shape=(None, Config.max_length), dtype=tf.bool, name="mask")
        self.dropout_placeholder = tf.placeholder(dtype=tf.float32, name="drop")
        ### END YOUR CODE
    ...
    def create_feed_dict(self, inputs_batch, mask_batch, labels_batch=None, dropout=1):
        ...
        Args:
            ...
            labels_batch: A batch of label data.184G
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(embedding, self.input_placeholder), [-1, Config.max_length, Config.n_features*Config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable(shape=(Config.hidden_size, Config.n_classes), dtype=tf.float32, name="U")
        b2 = tf.get_variable(shape=(Config.n_classes), dtype=tf.float32, name="b2")
        state = tf.zeros(shape=(tf.shape(x)[0], Config.hidden_size), dtype=tf.float32, name="h")
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                out_drop = tf.nn.dropout(output, dropout_rate)
                pred = tf.matmul(out_drop, U) + b2
                preds.append(pred)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=mask_pred, labels=mask_label))
        ### END YOUR CODE
        return loss
