...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        num_window = len(sentence)
        pad_sentence = window_size * [start] + sentence + window_size * [end]
        print(pad_sentence)
        for i in range(num_window):
            content = []
            window_i = pad_sentence[i: i + 2 * window_size+1]
            for j in window_i:
                content.extend(j)
            windowed_data.append((content, labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(embedding, self.input_placeholder),
                                [-1, Config.n_window_features * Config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                            shape=[Config.n_window_features * Config.embed_size, Config.hidden_size], name="W")
        b1 = tf.Variable(tf.zeros(Config.hidden_size), name="b1")

        U = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                            shape=[Config.hidden_size, Config.n_classes], name="U")
        b2 = tf.Variable(tf.zeros(Config.n_classes), name="b2")
        fc_1 = tf.matmul(x, W) + b1
        hidden = tf.nn.relu(fc_1)
        h_drop = tf.nn.dropout(hidden, keep_prob = self.dropout_placeholder)
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.pred, labels=self.labels_placeholder))
        ### END YOUR CODE
        return loss
